...
<<<<<<< HEAD
    ...
    it("Should create a payment and emit correct events", async function () {
        ...
        const tx = await paymentContract.createPayment(addr1.address, {
            value: amount,
        });
        const receipt = await tx.wait();

        expect(receipt.events[0].event).to.equal("PaymentCreated");
        expect(receipt.events[0].args[0].eq(ethers.BigNumber.from(1))).to.be
            .true;
        ...
        expect(receipt.events[1].args[0].eq(ethers.BigNumber.from(1))).to.be
            .true;
    });
    ...
    describe("Payment link functionality", function () {
        ...
        it("Should pay with a valid payment link", async function () {
            ...
            await paymentContract
                .connect(addr2)
                .payWithLink(requestId, { value: amount });
        ...
        it("Should fail to pay with an invalid payment link", async function () {
            ...
            await expect(
                paymentContract
                    .connect(addr2)
                    .payWithLink(invalidRequestId, { value: amount })
        ...
        it("Should fail to pay with incorrect amount", async function () {
            ...
            await expect(
                paymentContract
                    .connect(addr2)
                    .payWithLink(requestId, { value: incorrectAmount })
